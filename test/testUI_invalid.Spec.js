...
describe('Test gitAClue.js functions', () => {
  describe('Test options parameter with invalid parameters', () => {
    it('should return false for invalid options - null option', async () => {
      ...
      assert.equal(JSON.parse(result).error, 'option parameter is null, ' +
    ...
    it('should return false for invalid options - missing context keyword', 
      ...
      const result = await gitAClue.get(option);
      assert.equal(JSON.parse(result).error, 'context is null, undefined, ' +
        'or not a string');
    });
    ...
    it('should return false for invalid options - missing contextOwner ' +
      'keyword', async () => {
      const option = [
        { 
          context: 'repo',
      ...
      const result = await gitAClue.get(option);
      assert.equal(JSON.parse(result).error, 'contextOwner is null, ' +
    ...
    it('should return false for invalid options - missing contextName keyword', 
      ...
      const result = await gitAClue.get(option);
      assert.equal(JSON.parse(result).error, 'contextName is null, ' +
        'undefined, or not a string');
    });
    ...
    it('should return false for invalid options - invalid segment name', 
      ...
      const result = await gitAClue.get(option);
      assert.equal(JSON.parse(result).error, 'Invalid segments: gandalf');
    });

    it('should return false for invalid options - invalid context/segment ' +
      'name combination', async () => {
      const option = [
        {
          context: 'user',
          contextOwner: '',
      ...
      assert.equal(JSON.parse(result).error, 'Invalid segments: events');
    });
  });
});
